...
# Make within plot anotations 
anno <- data.frame(xstar = c(9, 79), ystar = c(3500, 3500),
                   lab = c("1", "2"),
                   lond = c("London", "UK metropolitan borough"))

anno2 <- data.frame(xstar = c(9, 25), ystar = c(3500, 3500),
                   lab = c("1", "2"),
                   lond = c("London", "UK metropolitan borough"))
...
a <- ggplot(all, aes(x = private_outdoor_space, y = TOTAL._CASES))+
  ...
  labs(colour = "Green space as\na percentage\nof area")+
  facet_grid(~lond, scales = "free")+ 
  geom_text(data = anno, aes(x = xstar,  y = ystar, label = lab), colour = "white", size = 5)
...
b <- ggplot(all, aes(x = greenspace_pc, y = TOTAL._CASES))+
  ...
  theme_black()+
  scale_y_continuous(name = 'Total cases')+
  scale_x_continuous(name = "Green space as a percentage of area")+
  theme(axis.title.y=element_text(size=15),axis.title.x=element_text(size=15), strip.text.x = element_text(size = 15))+
  scale_color_gradient(low = "darkgreen", high = "olivedrab1")+
  labs(colour = "Percentage of\nthe population\nwith a private\ngarden")+
  facet_grid(~lond, scales = "free")+ 
  geom_text(data = anno2, aes(x = xstar,  y = ystar, label = lab), colour = "white", size = 5)
